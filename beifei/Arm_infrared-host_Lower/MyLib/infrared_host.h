 ...
 * @param  hcan       CAN句柄 (如 &hcan1)，CAN硬件需已由工程初始化
 * @param  module_ids 模块ID数组, 如 (uint8_t[]){0x10, 0x11}
 * @param  count      模块数量
 * @retval true  成功
 * @retval false 失败(参数无效)
 ...
 * @param  module_id 模块ID，模块id的使用在can包发送前的id帧里体现
 * @param  data      发送数据
 * @param  length    数据长度(1~8)
 * @retval true  成功
 * @retval false 失败(超时/重试耗尽)
 * @warning 阻塞! 不可在中断中调用
 */
...
/**
 * @brief  读取模块最新缓存数据
 * @param  module_id 模块ID, 0xFF=读取第一个在线模块
 * @param  data      输出缓冲区(8字节), 可NULL
 * @param  length    输出数据实际长度, 可NULL
 * @retval true  数据有效
 * @retval false 模块不存在/数据无效
 * @code
 *   uint8_t buf[8], len;
